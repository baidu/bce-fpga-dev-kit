...
typedef struct XvcServerHandlers {
    ...
    void (*lock)(void *client_data, unsigned timeout);
    ...
    void (*state)(void *client_data, unsigned flags, unsigned state, unsigned long count);
 ...
 */
int xvcserver_start(const char *url, void *client_data, XvcServerHandlers *handlers);
