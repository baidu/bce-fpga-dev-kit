...
static int acpi_mcfg_check_entry(struct acpi_table_mcfg *mcfg,
					struct acpi_mcfg_allocation *cfg)
{
	...
	printk(KERN_ERR LOG_PREFIX "MCFG region for %04x:%02x-%02x at %#llx "
...
int __init pci_mmcfg_driver_init(void) {
	int result;
...
void __exit pci_mmcfg_driver_exit(void) {
	pci_mmcfg_arch_free();
}
