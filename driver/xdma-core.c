	...
	if (!lro->xvc_char.xvc_algo.type) {
		...
		if (user_config.config_space == BAR || user_config.config_space == AUTO) {
			...
			if (!bar_len) {
				...
				/*algo->offset.bar = pci_iomap(xvc_char->pci_dev, bar_index, map_len) + user_config.bar_offset;*/
...
static int xvc_open(struct inode *inode, struct file *file)
{
    ...
    printk(KERN_WARNING "cwndmiao debug %s\n", __func__);
	return 0;
}
...
static void dump_fops(struct xdma_dev *lro) {
    printk(KERN_WARNING "---------------------------------------------\n");
    printk(KERN_WARNING "xil_xvc_ioc_ops = %p\n", &xil_xvc_ioc_ops);
    printk(KERN_WARNING "lro->xvc_char_dev->cdev.ops = %p\n", lro->xvc_char_dev->cdev.ops);
    printk(KERN_WARNING "---------------------------------------------\n");
    return;
}
