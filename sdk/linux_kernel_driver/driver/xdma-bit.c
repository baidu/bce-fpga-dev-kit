...
static int hwicapWrite(struct xdma_dev *lro, const u32 *word_buf, int size)
{
    ...
    for (i = 0; i < size; i++) {
        ...
        write_register(value, lro->bar[lro->user_bar_idx] + XHWICAP_WF);
    }

    value = 0x1;
    write_register(value, lro->bar[lro->user_bar_idx] + XHWICAP_CR);
    for (i = 0; i < 10; i++) {
...
static long bitstream_icap_helper(struct xdma_dev *lro, const u32 *word_buffer, unsigned word_count)
{
    ...
    for (remain_word = word_count; remain_word > 0; remain_word -= word_written) {
        wr_fifo_vacancy = read_register(lro->bar[lro->user_bar_idx] + XHWICAP_WFV);
...
static long bitstream_mcap_setup(struct xdma_dev *lro, u32 *restore, bool design_switch)
{
    ...
    set |= MCAP_CTRL_MODE_MASK | MCAP_CTRL_IN_USE_MASK | MCAP_CTRL_DATA_REG_PROT_MASK;
...
static long bitstream_mcap_helper(struct xdma_dev *lro, const u32 *word_buffer, unsigned word_count)
{
    ...
    for (i = 0; i < word_count; i++) {
        ...
        /*
         * This delay resolves the MIG calibration issues we have been seeing with
...
long load_boot_firmware(struct xdma_dev *lro)
{
    ...
    if (err) {
        /* Could not find firmware with pcie id based name */
        printk(KERN_WARNING "Ignoring non existing firmware %s\n", fw_name);
        /* Now try the other possible name */
        snprintf(fw_name, sizeof(fw_name), "xilinx/xilinx_adm-pcie-ku3_1ddr_2_0.dsabin");
        err = request_firmware(&fw, fw_name, &lro->pci_dev->dev);
    }
